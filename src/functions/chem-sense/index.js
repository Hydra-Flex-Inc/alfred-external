...
const getChemSenseData = async (req, context) => {
  try {
    const out = {};
      ...
      .then((values) => {
        ...
        containers.forEach((container) => {
          ...
          container.characteristics = {
            mmInputMin:
              containerCharacteristics[container.container_type]?.mmInputMin ??
              1,
            mmPerGal:
              containerCharacteristics[container.container_type]?.mmPerGal ?? 1,
            containerGals:
              containerCharacteristics[container.container_type]?.gals ?? 1,
            refillGals:
              containerCharacteristics[container.container_type]?.refillGals ??
              1,
          };
